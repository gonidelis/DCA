...
private:
  ...
  hpx::lcos::local::mutex mutex_merge_;
  hpx::lcos::local::mutex mutex_queue_;
  hpx::lcos::local::condition_variable queue_insertion_;
...
void StdThreadQmciClusterSolver<QmciSolver>::startWalker(int id) {
    ...
    iterateOverLocalMeasurements(
        walker_index, [&](const int meas_id, const int tot_meas, const bool print) {
...

          {
            ...
            {
              std::unique_lock<hpx::lcos::local::mutex> lock(mutex_queue_);
  ...
  if (++walk_finished_ == parameters_.get_walkers()) {
    std::lock_guard<hpx::lcos::local::mutex> lock(mutex_queue_);
...
void StdThreadQmciClusterSolver<QmciSolver>::startAccumulator(int id) {
  ...
  {
    std::lock_guard<hpx::lcos::local::mutex> lock(mutex_merge_);
